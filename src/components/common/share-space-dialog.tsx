...
export const ShareSpaceDialog = ({ isOpen, setIsOpen }: ShareSpaceDialogProps) => {
                ...
                <DialogContent className="bg-muted p-0 gap-0">
                    ...
                    <div
                        ...
                        <div className="border rounded-lg flex items-center gap-1 py-1 px-1 ring-0 ring-theme-main justify-between focus-within:ring-1">
                            ...
                            <Button className="bg-theme-main font-bold py-0.5 px-2">Invite</Button>
                        </div>
                    </div>
                    <div className="flex items-center justify-between py-2 px-4 bg-white">
                        <Button variant={'ghost'} asChild>
                            <div className="flex">
                                ...
                                <a href="thisisalink" className="font-bold">Private Link</a>
                                <Info />
                            </div>
                        </Button>
                        <Button
                    ...
                    <div>
                        ...
                        <Collapsible open={collapsibleOpen} onOpenChange={(open) => setCollapsibleOpen(!open)} className="transition-transform p-0">
                                ...
                                <div className="flex items-center py-2 px-4 justify-between hover:bg-slate-200 rounded-xl">
                                    <div className="flex items-center gap-1">
                                        <IconCaretRightFilled
                                            size={14}
                                            className={cn("rounded-sm transition-transform duration-500 hover:bg-slate-300", collapsibleOpen ? 'rotate-90' : 'rotate-0')} />
                                        <div className="flex items-center gap-1">
                                            <User size={14}/>
                                            <p className="text-sm">People</p>
                                        </div>
                                    </div>
                                    <UsersArray onRemove={() => { }} visibleUsers={invited.slice(0, 3)} extraUsers={invited.slice(3)} />
                                </div>
                            </div>
                            <CollapsibleSpace members={members} invited={invited} onSelectInvited={onSelectInvited} />
                        </Collapsible>
                    </div>
                </DialogContent>
            </form>
...
const CollapsibleSpace = ({ members, invited, onSelectInvited }: SpaceDropdownProps) => {

    return (
        <CollapsibleContent
            className="overflow-hidden px-6 py-2 bg-muted transition-all data-[state=open]:animate-slide-down data-[state=closed]:animate-slide-up">
            <div className="flex items-center justify-between px-1">
                <p className="text-sm text-muted-foreground antialiased">{invited.length} invited</p>
                <div className="flex items-center gap-1">
                ...
                members.map((user) => {
                    ...
                    return (
                        <div className="flex items-center my-2 justify-between text-base">
                            <AssigneeAvatar assignee={user} displayName={true} onRemove={() => { }} />
                            <Switch checked={isInvited} onCheckedChange={() => onSelectInvited(user)} />
                        </div>
                    )
                })
            }
