...
export const AddTaskForm = ({ 
  ...
  return (
    <Form {...form}>
      <form onKeyDown={handleKeyDown} className={"space-y-2 bg-white p-2 rounded-xl"}>
        <div className="flex justify-end items-center mb-2 px-1">
          <span className="text-xs text-muted-foreground p-2 rounded bg-orange-300">⏎ Enter to add</span>
        </div>
        <FormField 
                ...
                <FormItem>
                                ...
                                <CommandList>
                                    <CommandEmpty>No Results Found.</CommandEmpty>
                                    {
                                        assignees.map((assignee) => {
                                            const selected = field.value?.some((a) => a.id === assignee.id);
                                            return (
                                                <CommandItem 
                                                 key={assignee.id}
                                                 onSelect={() => {
                                                    if (selected) {
                                                        field.onChange(field.value?.filter((a) => a.id !== assignee.id))
                                                    } else {
                                                        field.onChange([...(field.value || [] ), assignee])
                                                    }
                                                 }}>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                        {field.value?.map((assignee) => {
                            const name = assignees.find((a) => a.id === assignee.id)?.name;
                            return (
                        ...
                        <SelectContent>
                            {
                                priorities.map((priority,id) => (
                                    <SelectItem key={id} value={priority}>{priority}</SelectItem>
                                ))
                            }
                        </SelectContent>
                    </Select>
